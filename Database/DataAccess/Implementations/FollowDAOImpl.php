...
class FollowDAOImpl implements FollowDAO
{
    ...
    {
        $follwerRowData = $this->fetchFollowerUsers($userId);

        if ($follwerRowData === null) {
            return null;
        }
    ...
    private function fetchFollowerUsers(int $userId): ?array
    {
        $mysqli = DatabaseManager::getMysqliConnection();
    ...
    private function checkSelfFollowStatus(int $userId, int $followingId): bool
    {
    ...
    private function checkIsMutualFollowing(int $userId, int $partnerId): bool
    {
        ...
        if ($result[0]['is_mutual_follow'] === 0) {
            return false;
        }
